...
function playmusic {
  ...
  echo -e '\U1F3B6'
  find /home/paidipraneeth -name "*.mp3"
  echo "the list of the mp3 files are"
  find /home/paidipraneeth -name "*.mp4"
  echo -e '\U1F39E'
  echo "the list of the mp3 files are"
  echo -e '\U1F39E'
  find /home/paidipraneeth -name "*.mkv"
...
function search {
	echo " Enter file Name"
	read ser
	find /home/paidipraneeth -name "$ser"
	if read -t 5 c; then
	    echo "Closing: "
	    start
	   else
...
notify-send 'WELCOME TO OUR OPERATING SYSTEM' 'created by PAKGS!' -u critical -t 10000 -i checkbox-checked-symbolic
echo " Booooooottttiiinng UP"
sleep 0.5
start
